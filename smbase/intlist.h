// intlist.h
// list of integers (or other basic types that fit into void*)

// quarl 2006-05-31 - initial version based on SObjList
...
// forward declarations of template classes, so we can befriend them in IntList
// (not required by Borland C++ 4.5, but GNU wants it...)
template <class T> class IntListIter;
template <class T> class IntListMutator;
template <class T> class IntListIterNC;
...
class IntList {
private:
  friend class IntListIter<T>;
  friend class IntListMutator<T>;
  friend class IntListIterNC<T>;
...
public:
  // make shallow copies
  IntList(IntList const &obj)         : list(obj.list) {}
  IntList& operator= (IntList const &src)         { list = src.list; return *this; }

public:
  IntList()                            : list() {}
  ~IntList()                           {}    /* all items removed */
  ...
  // multiple lists
  void concat(IntList &tail)                       { list.concat(tail.list); }
  void appendAll(IntList const &tail)              { list.appendAll(tail.list); }
  void prependAll(IntList const &head)             { list.prependAll(head.list); }

  // steal
  void stealTailAt(int index, IntList &tail)       { list.stealTailAt(index, tail.list); }

  // equal items in equal positions
  bool equalAsLists(IntList const &otherList, Diff diff, void *extra=NULL) const
    { return list.equalAsLists(otherList.list, (VoidDiff)diff, extra); }
  int compareAsLists(IntList const &otherList, Diff diff, void *extra=NULL) const
    { return list.compareAsLists(otherList.list, (VoidDiff)diff, extra); }

  // last-as-set: comparisons (NOT efficient)
  bool equalAsSets(IntList const &otherList, Diff diff, void *extra=NULL) const
    { return list.equalAsSets(otherList.list, (VoidDiff)diff, extra); }
  bool isSubsetOf(IntList const &otherList, Diff diff, void *extra=NULL) const
  ...
  bool equalAsPointerLists(IntList const &otherList) const
    { return list.equalAsPointerLists(otherList.list); }
  bool equalAsPointerSets(IntList const &otherList) const
...
public:
  IntListMutator(IntList<T> &lst)     : mut(lst.list) { reset(); }
  ~IntListMutator()                    {}
  ...
  IntListMutator(IntListMutator const &obj)             : mut(obj.mut) {}
  IntListMutator& operator=(IntListMutator const &obj)  { mut = obj.mut;  return *this; }
...
#define SMUTATE_EACH_INTLIST(T, list, iter) \
  for(IntListMutator< T > iter(list); !iter.isDone(); iter.adv())
...
public:
  IntListIter(IntList<T> const &list) : iter(list.list) {}
  IntListIter(IntList<T> const &list, int pos) : iter(list.list, pos) {}
  ~IntListIter()                       {}

  void reset(IntList<T> const &list)   { iter.reset(list.list); }

  // iterator copying; generally safe
  IntListIter(IntListIter const &obj)             : iter(obj.iter) {}
  IntListIter& operator=(IntListIter const &obj)  { iter = obj.iter;  return *this; }

  // but copying from a mutator is less safe; see above
  IntListIter(IntListMutator<T> &obj)             : iter(obj.mut) {}
...
#define FOREACH_INTLIST(T, list, iter) \
  for(IntListIter< T > iter(list); !iter.isDone(); iter.adv())
...
public:
  IntListIterNC(IntList<T> &list) : iter(list.list) {}
  IntListIterNC(IntList<T> &list, int pos) : iter(list.list, pos) {}
  ~IntListIterNC()                     {}

  void reset(IntList<T> &list)         { iter.reset(list.list); }

  // iterator copying; generally safe
  IntListIterNC(IntListIterNC const &obj)             : iter(obj.iter) {}
  IntListIterNC& operator=(IntListIterNC const &obj)  { iter = obj.iter;  return *this; }

  // but copying from a mutator is less safe; see above
  IntListIterNC(IntListMutator<T> &obj)               : iter(obj.mut) {}
...
#define FOREACH_INTLIST_NC(T, list, iter) \
  for(IntListIterNC< T > iter(list); !iter.isDone(); iter.adv())


// iterate over the combined elements of two or more lists
template <class T>
class IntListMultiIter {
private:
  // all the lists
  IntList<T> **lists;               // serf array of serf list pointers
  int numLists;                      // length of this array

  // current element
  int curList;                       // which list we're working on
  IntListIter<T> iter;              // current element of that list
...
public:
  IntListMultiIter(IntList<T> **L, int n)
    : lists(L),
      numLists(n),
      curList(0),
      iter(*(lists[0]))
...
template <class T>
void IntListMultiIter<T>::normalize()
{
...
// (tweak to ensure objlist.h and intlist.h both change)
