  ...
  // some fields (re-)initialized by 'clearAllStackNodes'
{
  ...
  if (ENABLE_EEF_COMPRESSION != !!(tables->errorBits)) {
    xfailure(stringc
      << "The GLR parser core was compiled with EEF compression "
      << (ENABLE_EEF_COMPRESSION? "enabled" : "disabled")
      << ", but the parse tables generated by Elkhound have it "
      << (tables->errorBits? "enabled" : "disabled"));
  }
  if (ENABLE_GCS_COMPRESSION != !!(tables->actionIndexMap)) {
    xfailure(stringc
      << "The GLR parser core was compiled with GCS compression "
      << (ENABLE_GCS_COMPRESSION? "enabled" : "disabled")
      << ", but the parse tables generated by Elkhound have it "
      << (tables->actionIndexMap? "enabled" : "disabled"));
  }
  if (ENABLE_GCS_COMPRESSION) {
    xassert(tables->actionRowPointers);
  }
}
