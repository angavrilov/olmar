...
// check to see if the given type would be different after
// signature normalization
bool TypeFactory::hasDifferentSignature(Type const *type)
{
  switch (type->getTag()) {
    default: xfailure("bad tag");
    ...
    case Type::T_POINTER:
      return hasDifferentSignature(type->asPointerTypeC()->atType);

    case Type::T_FUNCTION: {
      FunctionType const *ft = type->asFunctionTypeC();
      SFOREACH_OBJLIST(Variable, ft->params, iter) {
        Type const *paramType = iter.data()->type;
                                            
        // if this parameter has any toplevel cv flags, or if
        // it has parameters with such flags deeply, then the
        // signature of 'type' as a whole is different
        if ((paramType->getCVFlags() != CV_NONE) ||
            hasDifferentSignature(paramType)) {
          return true;
        }
      }
      return false;
    ...
    case Type::T_ARRAY:
      return hasDifferentSignature(type->asArrayTypeC()->eltType);
      
    case Type::T_POINTERTOMEMBER:
      return hasDifferentSignature(type->asPointerToMemberTypeC()->atType);
  }
}


// Given a type, return one with all toplevel cv flags stripped off
// any parameters inside function types (might return the same type
// given to it); only such types will appear in the environment and in
// overload sets.
//
...
// refer to functions.
//
// Note that cv flags must be stripped deeply; see footnote 112 of cppstd,
// in section 13.1 para 3.
//
// This function is only called once 'hasDifferentSignature' returns true,
// so I don't try to be efficient about re-using type objects, since
// this function should in fact be called only very rarely.
Type *TypeFactory::normalizeSignature(Type *type, CVFlags cvmask)
{
  switch (type->getTag()) {
    default: xfailure("bad tag");
    ...
    case Type::T_ATOMIC: {
      ...
      return makeCVAtomicType(SL_UNKNOWN, at->atomic, (at->cv & cvmask));
    }

    case Type::T_POINTER: {
      PointerType *pt = type->asPointerType();
      return makePointerType(SL_UNKNOWN, pt->op, (pt->cv & cvmask),
                             normalizeSignature(pt->atType));
    }

    case Type::T_FUNCTION: {
      FunctionType *ft = type->asFunctionType();
      FunctionType *ret = makeSimilarFunctionType(SL_UNKNOWN, ft->retType, ft);

      // re-create the parameter list, normalized
      SFOREACH_OBJLIST_NC(Variable, ft->params, iter) {
        Variable *param = iter.data();

        // pass CV_NONE as the 'cvmask' to suppress toplevel cv flags
        // in the parameter types
        Variable *newParam =
          makeVariable(param->loc, param->name,
                       normalizeSignature(param->type, CV_NONE), param->flags);
        ret->addParam(newParam);
      }
      ret->doneParams();
    ...
    case Type::T_ARRAY: {
      ...
      return makeArrayType(SL_UNKNOWN, normalizeSignature(at->eltType),
                           at->size);
    }

    case Type::T_POINTERTOMEMBER: {
      PointerToMemberType *ptm = type->asPointerToMemberType();
      return makePointerToMemberType(SL_UNKNOWN, ptm->inClass, (ptm->cv & cvmask),
                                     normalizeSignature(ptm->atType));
    }
  }
}
