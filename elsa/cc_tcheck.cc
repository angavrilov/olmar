...
void Function::tcheck(Env &env, Variable *instV)
{
  ...
  // get return type
  CType *retTypeSpec = retspec->tcheck(env, dflags, LF_NONE);
...
void Declaration::tcheck(Env &env, DeclaratorContext context)
{
  ...
  CType *specType = spec->tcheck(env, dflags, LF_NONE);
...
// --------------------- TypeSpecifier --------------
CType *TypeSpecifier::tcheck(Env &env, DeclFlags dflags, LookupFlags lflags)
{
  env.setLoc(loc);

  CType *t = itcheck(env, dflags, lflags);
...
// 7.1.5.2
CType *TS_name::itcheck(Env &env, DeclFlags dflags, LookupFlags lflags)
{
...
CType *TS_simple::itcheck(Env &env, DeclFlags dflags, LookupFlags lflags)
{ 
...
CType *TS_elaborated::itcheck(Env &env, DeclFlags dflags, LookupFlags lflags)
{
  env.setLoc(loc);
...
CType *TS_classSpec::itcheck(Env &env, DeclFlags dflags, LookupFlags lflags)
{
  env.setLoc(loc);
...
CType *TS_enumSpec::itcheck(Env &env, DeclFlags dflags, LookupFlags lflags)
{
  env.setLoc(loc);
...
// NULL.
static NamedAtomicType *isUninstClassTemplate(CType *t)
{
  if (!t->isCVAtomicType()) {
    return NULL;
  }
